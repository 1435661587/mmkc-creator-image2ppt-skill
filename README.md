...
`mmkc-creator-image2ppt-skill` 是一个 Codex skill，用来把图片、截图、HTML 或 SVG 设计稿转换成可编辑的 PowerPoint `.pptx` 文件。它适合 PPT 截图还原、信息图转幻灯片、AI 生图转可编辑页面、HTML/SVG 代码式页面转 PPTX，以及“把平面图片拆成组件图片 + 文本框 + 原生形状再重建”的工作流。
...
```text
使用 mmkc-creator-image2ppt-skill 把这张 PPT 图片转成真正的 pptx 文件。
...
```text
使用 mmkc-creator-image2ppt-skill 把这个 HTML/SVG 页面转成可编辑 PPTX。
